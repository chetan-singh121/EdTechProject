...
exports.createCourse=async(req,res)=>
    {
        try{
            ...
            console.log("course desc = ",courseDescription);
            ...
            //validation
            if(!courseName || !courseDescription ||! whatYouWillLearn ||!price ||!thumbnail ||!tag.length||!instructions.length||!category)
                {
            ...
            //TODO: verfiy userId and instructorDeatils._id are same or diffenet
            // ye to maine pehle hi pata laga lia tha ki ye same hai,

            if(!instructorDetails)
                {
                    return res.status(404).json({
                        success:false,
                        message:"jo given userId hai us se instructor Details not found",
                    });
                }
            
            //check given tags is valid or not.
            // --> vaise UI se aaya hai to galat ni hoga , but postman se hum test krnege to galat de skta hai..
            ...
            //update user , yaha user instructor hai,
            //add teh new course to user schema of instructor
         ...
         const categoryDetails2 = await Category.findByIdAndUpdate(
          ...
          console.log("new course added to category", categoryDetails2)
            ...
            if(!courseDetails)
                {
                    return res.status(400).json({
                        success:false,
                        message:`courld not find course with id = ${courseId}`,
                        
                    })
                }
            
            //course details return response
            return res.status(200).json({
                success:false,
